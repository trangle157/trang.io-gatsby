...
const listenForMutations = callback => {
  ...
  const emitQueryChange = event => {
    callback({
      type: `QUERY_FILE_CHANGED`,
      payload: event
    });
  };
  ...
  _redux.emitter.on(`SOURCE_FILE_CHANGED`, emitFileChange);

  _redux.emitter.on(`QUERY_FILE_CHANGED`, emitQueryChange);

  return function unsubscribeFromMutationListening() {
    _redux.emitter.off(`ENQUEUE_NODE_MUTATION`, emitMutation);

    _redux.emitter.off(`SOURCE_FILE_CHANGED`, emitFileChange);

    _redux.emitter.off(`WEBHOOK_RECEIVED`, emitWebhook);

    _redux.emitter.off(`QUERY_FILE_CHANGED`, emitQueryChange);
  };
};
